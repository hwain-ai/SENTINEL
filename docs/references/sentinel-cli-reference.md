...
`check`는 각 모듈 안의 CRAP·mutation을 기본으로 병렬 실행한다. `--execution-mode parallel`은 기본값을 명시하며, `--execution-mode sequential`은 두 검사를 순서대로 실행한다. 모듈 간 실행 순서는 유지한다. 두 방식의 검사 범위·점수 기준은 같다. Java의 함수 위치 선택은 두 검사 전에 소스 분석으로 확정한다.

도구 요청에는 `executionMode`를 보내고 응답에서 같은 값을 확인한다. 응답이 없거나 요청과 다르면 `executionModeNotAcknowledged` 진단과 종료 6을 반환한다. 예전 도구가 순차 실행한 결과를 기본 병렬 실행으로 표시하지 않는다. 새 옵션을 사용하려면 이를 지원하는 언어 어댑터가 필요하며, 개발 사본은 기존 공개 승인 목록에 자동으로 추가되지 않는다.

기본 병렬 실행을 지원하는 어댑터는 Python·TypeScript `0.1.5`, Java `0.1.6`이다. 기존 프로젝트에서 `executionModeNotAcknowledged`가 나오면 [도구 갱신 안내](../../README.md#승인된-도구-버전-갱신)에 따라 실행기와 언어 도구를 함께 갱신한다. 승인 목록에는 해당 소스 커밋의 CI가 성공한 버전만 추가한다.

사용자 취소·명시한 시간 제한 초과 시에는 도구에 먼저 정리를 요청하고, 정리를 완료하지 못하면 강제 종료 후 오류로 처리한다. 이 정리 대기 시간은 검사 자체의 기본 실행 시간 제한을 추가한 것이 아니다.
...
Linux 전용 Go·OCI 경로의 보호된 파일 준비는 `/proc/self/fd`로 이미 열어 둔 폴더를 확인한 뒤 권한을 설정합니다. 새 커널의 `fchmodat2`를 필수로 요구하지 않습니다. 원래 폴더 경로가 교체돼도 열린 폴더만 대상으로 하며, 참조가 일치하지 않거나 안전하게 접근할 수 없으면 `contentRootFailed`로 거부합니다. Python·TypeScript·Java의 일반 검사와 이 Linux 전용 경로의 지원 범위는 구분합니다.
