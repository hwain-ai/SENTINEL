...
|항목|현재 확인한 범위|남은 확인|
|---|---|---|
|언어 검사기|Python·TypeScript 0.1.2, Java 0.1.3을 main에 게시했고 각각 네 플랫폼 CI를 통과했다. 실제 커밋·지문·CI를 대조해 승인했으며 이전 승인 6항목은 해제했다.|완료. 기존 사용자는 CLI 승인 목록과 언어 도구를 함께 갱신한다.|
|다중 언어 설정|새 다중 언어 설정에 명시적 `--module-root`를 사용한다. 모듈별 설정·의존성 경로를 분리하고 기존 ID·기준값을 보존한다.|설치 중 편집 보존과 이동 폴더 매핑까지 회귀 검증을 마쳤다.|
|미검사 인증|실행기가 생략한 `noChanges`는 종료 0이지만 `certified=false`다. 새 어댑터의 README-only 실제 3사례도 noChanges다.|완료. 두 호스트도 변경 없음과 실제 품질 통과를 구분했다.|
|실제 언어 시험|기존 18사례, 새 버전 실험 21사례와 정식 승인 뒤 기본 21사례를 완료했다. 새 버전은 README-only도 noChanges이며 원본 21/21을 보존했다. 구버전 세 묶음은 실행 없이 거부했다.|완료. 최종 Python 늦은 취소의 1.325초 정리 지연과 이전 관측을 기록했다.|
|Codex 호스트|사용자 범위 설치·활성화·스킬 발견·WSL doctor·실험 실행과 정식 승인 뒤 세 언어 기본 check 및 변경 없음 check를 확인했다. 원본도 보존했다.|완료. 실제 검사와 미검사 인증, 실험 미승인을 정확히 구분했다.|
|Claude Code 호스트|Windows 재로그인 뒤 설치된 스킬의 WSL doctor, 새 버전 세 언어 기본 check와 변경 없음 check를 확인했다. 실제 검사와 미검사 인증을 구분했고 원본도 보존했다.|완료. WSL의 별도 Claude 로그인은 필요하지 않았다.|
|공개 안내|지원 언어 안내는 Python·TypeScript·Java로 한정한다. Linux/WSL 지원과 macOS 통합 설치 미지원 경계를 명시했다. 공개 5개 저장소의 main 게시와 CI가 완료됐다.|완료. 최종 기본 실행 기록·새 승인 목록·구버전 갱신 절차를 반영했다.|

이번 요청의 다섯 단계는 완료했다. 이후 확장 범위는 더 큰 사용자 프로젝트·다른 Linux CPU와 커널 검증이다. 현재 WSL 커널의 전체 시험 한계와 Python 취소 후 짧은 정리 지연은 검증 기록에 남겨 두며 완료 표시로 지우지 않는다.
...
2026-09-16 사용자는 다중 언어·미검사 표시 수정, WSL 준비, 두 호스트 설치와 실제 호출, 세 언어의 실패·취소 시험, 진행 문서·플랫폼 안내 갱신을 승인했다. 같은 세션에서 이미 승인된 설치·검사를 다시 확인받지 않는다. 비지원 언어 저장소의 비공개 전환과 공개 문서의 언급 제거도 승인됐고 완료했다. 이후 사용자의 명시적 main 게시 지시로 공개 5개 저장소를 게시했다.
