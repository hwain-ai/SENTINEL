...
[주요 기능](#주요-기능) · [원본을 보존하는 검사 방식](#원본을-보존하는-검사-방식) · [기술 스택](#기술-스택) · [사용 가이드](#사용-가이드) · [실행 명령](#에이전트가-실행하는-명령) · [JSON 읽기](#json-결과-읽기) · [종료 코드·상태](#종료-코드와-status) · [기존 도구와 비교](#tools-comparison)

## 주요 기능

| 기능 | 확인할 수 있는 내용 |
|---|---|
| CRAP 검사 | 소스 코드의 로직을 바꾸지 않고, 함수의 복잡도와 테스트가 실행한 코드 범위를 함께 평가합니다. 낮을수록 좋으며 기본 상한은 **8**입니다. |
| Mutation 검사 | 검사용 복사본의 연산자나 값을 일부러 바꾼 코드인 **변이**를 테스트가 발견하는지 확인합니다. 기본 최소 탐지율은 **90%**입니다. |
| 원본 소스 보존 | 두 검사 모두 별도 임시 폴더에 만든 프로젝트 복사본에서 테스트를 실행합니다. 검사용 변경을 원본 소스에 덮어쓰지 않습니다. |
| 범위 선택 | 기능 파일, 특정 함수, 실행할 테스트를 지정합니다. Git 변경분이나 설정된 전체 범위도 검사합니다. |
| 위치와 근거 | 파일·함수별 점수, 기준 미달 여부, 발견하지 못한 변이의 위치와 상태를 반환합니다. |
| JSON출력  | 세 언어에서 같은 필드로 검사 범위·품질 판정·실행 오류를 읽습니다. |

품질 통과는 **검사한 범위가 CRAP·mutation 기준을 충족했다는 뜻**입니다. 제품의 모든 요구사항 충족이나 모든 버그의 부재를 뜻하지 않습니다.

## 원본을 보존하는 검사 방식

SENTINEL은 **프로젝트의 소스 코드·테스트·설정 파일을 운영체제의 임시 폴더에 복사한 뒤, 복사본에서 테스트를 실행합니다.** CRAP 검사와 mutation 검사는 이 복사본을 사용하는 방식이 다릅니다.

**기본 실행은 병렬입니다.** 같은 검사 요청의 CRAP과 mutation을 별도 작업 폴더에서 함께 시작하고, 두 결과를 모아 판정합니다. 순차 실행하려면 `--execution-mode sequential`을 붙입니다. 여러 프로젝트 모듈을 한꺼번에 병렬 실행하는 옵션은 아닙니다.

```sh
# 기본값: CRAP과 mutation을 병렬로 검사합니다.
sentinel check --file src/pricing.py --tests tests/test_pricing.py
# 메모리가 부족하거나 테스트가 같은 DB·포트를 사용하면 순차 실행을 선택합니다.
sentinel check --file src/pricing.py --tests tests/test_pricing.py --execution-mode sequential
```

- **CRAP은 소스 코드의 로직을 바꾸지 않습니다.** 코드를 읽어 조건문·반복문 등에 따른 복잡도를 계산하고, 테스트를 실행해 실제로 실행된 코드 범위인 **커버리지**를 측정합니다. 두 값을 함께 사용해 CRAP 점수를 계산합니다.
- **Mutation은 복사본의 로직을 일부러 바꿉니다.** 예를 들어 곱셈을 나눗셈으로 바꾼 뒤 같은 테스트를 실행합니다. 테스트가 그 오류를 놓치는지 확인해, 테스트가 오류를 찾아내는 능력을 평가합니다.

```mermaid
flowchart TD
    O["내 프로젝트<br/>소스 코드 · 테스트 · 설정 파일"]

    subgraph CRAP["CRAP 검사: 소스 코드의 로직 유지"]
        C1["임시 폴더에<br/>CRAP 검사용 복사본 생성"]
        C2["코드를 읽어 복잡도 계산<br/>테스트를 실행해 커버리지 측정"]
        C3["복잡도와 커버리지로<br/>CRAP 점수 계산"]
        C1 --> C2 --> C3
    end

    subgraph MUTATION["Mutation 검사: 복사본의 로직을 일부러 변경"]
        M1["임시 폴더에<br/>mutation 검사용 복사본 생성"]
        M2["기존 테스트가 통과하는지 확인<br/>복사본의 연산자·조건 등을 변경"]
        M3["같은 테스트를 다시 실행<br/>만들어 넣은 오류를 잡는지 확인"]
        M4["변이 탐지율과<br/>놓친 오류의 위치 수집"]
        M1 --> M2 --> M3 --> M4
    end

    O -->|"병렬 시작 · 파일 복사"| C1
    O -->|"병렬 시작 · 파일 복사"| M1
    C3 --> R["점수 · 파일 · 함수 · 실패 위치를 모아 반환<br/>검사용 임시 폴더 정리"]
    M4 --> R
    O -.-> K["원본 소스 유지<br/>검사용 변경을 덮어쓰지 않음"]
...
CRAP도 실제 테스트를 실행하므로 측정 보고서나 빌드 결과가 생깁니다. 이 작업을 원본 프로젝트와 분리하기 위해 CRAP 검사에도 복사본을 사용합니다. 그림은 두 검사가 정상적으로 진행되는 경우이며, 기존 테스트부터 실패하거나 측정에 필요한 결과를 얻지 못하면 해당 문제를 보고합니다.

### 복사본이 만들어지는 위치

다음은 **Linux에서 Python 프로젝트를 기본 병렬 방식으로 검사할 때의 경로 예시**입니다. `abc123` 같은 뒷부분은 설명용 이름이며, 실제 임시 폴더 이름은 실행할 때 자동으로 만들어집니다.
...
CRAP 검사용 복사본: 소스 코드의 로직을 유지하며 측정
/tmp/sentinel-parallel-abc123/0/sentinel-py-coverage-def456/project/
├── src/pricing.py
└── tests/test_pricing.py

Mutation 검사용 복사본: 이 작업 폴더 안에서 변이를 만들고 검사
/tmp/sentinel-parallel-abc123/1/sentinel-py-mutmut-ghi789/project/
├── src/pricing.py
└── tests/test_pricing.py
```

TypeScript와 Java도 운영체제의 임시 폴더에 검사용 복사본을 만듭니다. 실제 경로와 하위 폴더 구성은 언어·운영체제에 따라 다르며, 검사 후 임시 폴더를 정리합니다.

### 검사 대상 프로젝트의 의존성

소스가 사용하는 외부 패키지는 검사 전에 준비되어 있어야 합니다. 검사 도구 설치와 프로젝트 패키지 준비는 별개입니다.

| 언어 | 프로젝트 패키지 준비와 사용 |
|---|---|
| Python | `setup --python-requirements requirements.txt`로 준비한 `.sentinel-deps`를 각 검사 복사본에 함께 복사합니다. 기존 `.venv`를 복사하거나 자동으로 사용하지 않습니다. |
| TypeScript | 검사할 모듈의 `node_modules`에 프로젝트 패키지를 먼저 설치합니다. 검사 복사본은 이 패키지를 링크로 참조하며, 패키지 내용을 다시 복사하지 않습니다. Vitest·Vite·변이 검사 도구는 검사기에 고정된 버전을 사용합니다. 검사 중 패키지를 자동 설치하지 않습니다. |
| Java | `setup --language java --java-dependencies`로 준비한 `.sentinel-m2`를 두 검사가 함께 참조합니다. 이 폴더는 복사하지 않습니다. 프로젝트 저장소가 없으면 검사기의 Maven 저장소를 사용하며, 필요한 패키지가 없으면 설치·실행 오류가 됩니다. |

병렬 실행 중에는 두 작업의 복사본과 메모리가 동시에 필요합니다. 실행 오류가 나면 진행 중인 다른 검사를 중단하고 정리합니다. CRAP 점수나 mutation 탐지율이 기준에 미달한 경우에는 두 검사 결과를 모두 모아 보고합니다. 검사 도중 원본 코드가 바뀌면 그 결과를 통과로 처리하지 않습니다.

**검사용 변경과 실제 코드 개선은 별도 단계입니다.** `check`는 검사 결과만 보고합니다. 기본 `sentinel` 호출에서는 Claude Code·Codex가 결과를 읽고 원본 코드와 테스트를 개선합니다. SENTINEL은 수정된 프로젝트의 새 복사본을 만들어 재검사합니다.
...
flowchart TD
    ...
    S["SENTINEL<br/>검사용 복사본에서 CRAP·mutation 검사<br/>CRAP은 로직 유지 · mutation은 복사본 변경"]
    J["JSON: exitCode: 2<br/>selection: partial<br/>status: qualityFailed<br/>점수와 실패 위치 포함"]
    R["코딩 에이전트<br/>결과 설명"]
    F["기본 호출은<br/>기준 미달 시 수정·재검사<br/>check는 결과 보고로 종료"]
    U --> A --> C --> S --> J --> R
    R --> F --> C
```
...
| 기본 호출 또는 점수 확인용 check를 선택합니다. | 기본 호출은 기준 통과까지 수정하며 check는 결과만 보고합니다. |

관련 테스트를 고르는 주체는 코딩 에이전트입니다. SENTINEL은 전달받은 파일과 테스트를 실행하고 측정값을 돌려줍니다.

## 점수 목표 참고 자료

기본값은 Prime Agent와 DeepSeek로 6개 목표 조건을 총 54회 비교 테스트한 결과, 기능 해결률이 가장 높았던 그룹 중 작업 시간이 가장 짧았던 **CRAP 8 이하·mutation 90% 이상**으로 잡았습니다. [CRAP·mutation과 기능 해결의 상관관계, 황화인(PDF 8쪽)](docs/evidence/crap-mutation-function-study.pdf)
...
실제 검사는 **Linux·macOS(Intel/Apple Silicon) 또는 Windows의 WSL2 Ubuntu**에서 실행합니다. 실행 환경에는 Git, Python 3.9+와 가상환경 생성 기능이 필요합니다. Windows에서는 `wsl --list --verbose`로 WSL2 배포판을 확인하세요.
...
새 대화에서 사용하는 호스트의 명령을 입력합니다. 이 문서의 스킬 구성은 플러그인 **0.5.0** 기준입니다.
...
아래는 Linux·macOS 또는 WSL 안에서 처음 설치하는 명령입니다. 설치 폴더가 이미 있으면 [갱신 안내](#승인된-도구-버전-갱신)를 따릅니다. Git·Python·가상환경 기능이 없으면 먼저 운영체제의 패키지 관리자로 준비합니다.
...
| 스킬 이름 | 하는 일 |
|---|---|
| `sentinel` | 기존 CRAP·mutation 기준을 만족할 때까지 코드·테스트 수정과 재검사 |
...
/sentinel:sentinel 설정된 전체 범위가 통과할 때까지 수정해 줘.
/sentinel:check src/pricing.py의 현재 점수와 실패 위치만 보여 줘.
/sentinel:version
/sentinel:update
```

새 설정의 기본값은 CRAP 8 이하·mutation 90% 이상이며 기존 프로젝트 기준이 있으면 그 값을 유지합니다. 기본 호출에 다른 목표 수치를 명시할 수도 있습니다. 에이전트는 기준을 낮추거나 검사 대상을 제외해서 통과시키지 않습니다. 중간에 좁은 범위로 확인하더라도 마지막에는 요청한 원래 범위로 다시 검사합니다.

`version` 스킬은 실행기 버전 번호(`sentinel --version`)와 프로젝트의 도구 설치 상태(`sentinel version`)를 확인합니다. 실행기 0.4.0 이상을 사용합니다. 설치·업데이트·품질 검사를 자동으로 실행하지 않습니다. `update`는 프로젝트 코드·테스트·품질 기준을 유지하며 공식 배포판으로 갱신합니다.
