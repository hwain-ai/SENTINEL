...
| 단계 | 사용자 컴퓨터에 설치되는 것 |
|---|---|
| 플러그인 설치 | 에이전트가 읽는 SENTINEL 사용 지침과 플러그인 설정. 호스트가 설치 목록을 가져오면서 저장소 캐시도 받을 수 있음 |
| 첫 실행 준비 | SENTINEL 실행기 소스·Python 실행 환경, 필요한 언어 검사기 소스와 SDK |
| 프로젝트 검사 | 설치한 로컬 실행기가 코드와 테스트를 검사하고 JSON 반환 |

SENTINEL은 현재 로컬 설치형 도구입니다. 원격 서버의 검사 기능만 호출하는 서비스가 아닙니다. 모든 언어 검사기를 반드시 받는 것은 아니며, 준비할 언어를 선택합니다.
